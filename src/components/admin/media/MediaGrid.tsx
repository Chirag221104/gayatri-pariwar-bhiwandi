    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
                    <button
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${!currentFolderId ? "bg-orange-500 text-white shadow-lg shadow-orange-500/20" : "text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-zinc-800"}`}
                    >
                    ...
                    {currentFolderId && (
                        <>
                            ...
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded-lg border border-orange-100 dark:border-orange-900/30">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <div className="relative flex-1 sm:max-w-xs">
                        ...
                        <input
                            ...
                            className="w-full bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl py-2 pl-10 pr-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-orange-500/20 outline-none transition-all"
                        />
                    </div>
                    <button
                        onClick={() => setIsCreatingFolder(true)}
                        className="p-2 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-400 rounded-xl transition-all border border-slate-200 dark:border-zinc-800 shadow-sm"
                        title="New Folder"
                    >
            ...
            {(isCreatingFolder || editingItem) && (
                <div className="bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20 p-4 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top duration-200 shadow-sm">
                    ...
                    >
                        <input
                            ...
                            className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/20 outline-none"
                        />
                        ...
                        <button
                            ...
                            className="p-2 bg-white dark:bg-zinc-900 text-slate-400 dark:text-zinc-500 rounded-lg hover:text-slate-600 dark:hover:text-white border border-slate-200 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all"
                        >
                            <X className="w-4 h-4" />
                        </button>
                    </form>
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-6 space-y-8">
                ...
                {folders.length > 0 && (
                    <div className="space-y-3">
                        <h4 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 px-1">
                            Folders
                            <span className="h-px flex-1 bg-slate-200 dark:bg-zinc-800"></span>
                        </h4>
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                            {folders.map((folder: MediaFolder) => (
                                <div
                                    key={folder.id}
                                    className="group relative bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-3 hover:border-orange-500 dark:hover:border-orange-500 hover:shadow-md transition-all cursor-pointer overflow-hidden"
                                    onClick={() => onFolderChange(folder.id)}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-orange-50 dark:bg-zinc-950/30 flex items-center justify-center text-orange-500 group-hover:scale-110 transition-transform">
                                            <Folder className="w-4 h-4 fill-current" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate group-hover:text-slate-900 dark:group-hover:text-white">{folder.name}</p>
                                        </div>

                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setEditingItem({ id: folder.id, name: folder.name, type: 'folder' }); }}
                                                className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded"
                                            >
                    ...
                    {filteredFiles.length > 0 ? (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4">
                            {filteredFiles.map((file: MediaFile, index: number) => (
                                <div
                                    key={file.id}
                                    className="group relative bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden hover:border-orange-500 dark:hover:border-orange-500 hover:shadow-lg transition-all cursor-pointer"
                                    onClick={() => setPreviewIndex(index)}
                                >
                                    <div className="aspect-[4/3] bg-slate-100 dark:bg-zinc-800 flex items-center justify-center overflow-hidden relative">
                                    ...
                                    <div className="p-3">
                                        <p className="text-[11px] font-bold text-slate-700 dark:text-slate-200 truncate w-full mb-1 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">{file.name}</p>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 dark:bg-zinc-900/50 border border-dashed border-slate-200 dark:border-zinc-800 rounded-3xl">
                ...
                {filteredFiles.length === 0 && folders.length === 0 && !loading && (
                    <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-3xl bg-slate-50/50 dark:bg-zinc-900/20">
                        <Folder className="w-12 h-12 text-slate-300 dark:text-slate-700 mb-4" />
                        <h3 className="text-slate-900 dark:text-white font-bold text-lg">Empty directory</h3>
                    ...
                    <div className="flex-1 w-full flex items-center justify-between px-4 sm:px-10 relative overflow-hidden group/view">
                        ...
                        <div className="flex-1 h-full flex items-center justify-center p-4 md:p-20">
                            {(filteredFiles[previewIndex].type === 'image' || filteredFiles[previewIndex].type?.startsWith("image/") || filteredFiles[previewIndex].mimeType?.startsWith("image/")) ? (
                                ...
                                <div className="flex flex-col items-center gap-6 p-20 bg-zinc-800/50 rounded-3xl border border-white/5">
